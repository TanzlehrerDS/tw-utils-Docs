...
Das Bunker-Information-System verwaltet eure defensiven Bunker im Stamm: Spieler stellen Bunker-Anfragen für ihre Dörfer, TWU-Mods genehmigen oder lehnen sie ab, später kann pro Bunker aufgestockt (Top-Up) oder der Bunker aufgelöst werden. Daneben bietet das Modul einen Leader-View mit Bunker-Health-Ampel und FlexControl-Suche sowie einen eigenen Kanal, um feindliche Bunker zu melden und zu verwalten.

## 1. Kanäle des Moduls

Nach der [Installation](modul-verwaltung.md) legt der Bot die Kategorie `🧱 BUNKER-INFORMATION-SYSTEM` mit vier Kanälen an:

- `#⚫-request-bunker` — zentraler Anfrage-Kanal mit den Buttons zum Anfragen, Top-Up, Anzeigen und Auflösen eigener Bunker
- `#⚫-leaderview-bunker` — Leader-View mit Bunker-Health-Übersicht aller genehmigten Bunker
- `#⚫-leaderview-flexbunker` — Leader-View mit der FlexControl-Suche nach potenziellen Flex-Bunkern
...
Sobald jemand eine Anfrage oder ein Top-Up stellt, legt der Bot pro Koordinate **einen eigenen Kanal** in derselben Kategorie an (z. B. `❓-bunker-for-500-501-Spielername` bzw. `❓-top-up-500-501`). In diesem Per-Request-Kanal läuft der gesamte Genehmigungs-Workflow.
...
Nach erfolgreicher Genehmigung erhält der Anfragende eine Discord-Direktnachricht mit dem Titel `Bunker Approved` (bzw. `Top-Up Approved`). Die DM enthält alle Eckdaten plus einen fertigen SD-Befehl-String zur direkten Übernahme auf tw-utils.net. Der Anfrage-Kanal wird im Anschluss automatisch gelöscht.

![Genehmigungs-DM](../assets/discordbot/bunker-information-system/09_approve_bunker_summary_as_discord_dm.png){ .screenshot }

Statt zu genehmigen, können TWU-Mods die Anfrage mit `Admin: Reject` ablehnen. Auch hier wird der Anfrage-Kanal nach dem Klick gelöscht.
...
!!! info "Genehmigung mit Größen-Override"
    Im Approval-Modal kann der TWU-Mod eine **abweichende Größe** vergeben — etwa weil das Dorf eine andere Stärke aufbauen soll als ursprünglich angefragt. Diese Größe wird zur verbindlichen Soll-Größe und ist Basis für die Bunker-Health-Ampel im Leader-View.

!!! info "SD-Befehl aus Approval-DM kopieren"
    Die `Bunker Approved`-DM enthält am Ende einen fertig formatierten SD-Befehl-String (`{coord} {units}"{Spieler}"request tw-utils.net""`). Diesen Block kannst du **direkt** in den SD-Import auf tw-utils.net einfügen, ohne ihn manuell zusammenzubauen.
...
![Neuer Top-Up-Kanal in der Kategorie](../assets/discordbot/bunker-information-system/12_request_topup_new_channel_created.png){ .screenshot }

Im Kanal postet der Bot eine Top-Up-Zusammenfassung. Der Genehmigungs-Workflow ist identisch zu Abschnitt 3 — bei Genehmigung erhält der Anfragende eine DM mit dem Titel `Top-Up Approved`, und die zusätzliche Stärke wird auf die bestehende Soll-Größe addiert.

![Top-Up-Zusammenfassung im Kanal](../assets/discordbot/bunker-information-system/13_request_topup_new_channel_request_sumamry.png){ .screenshot }
...
Wenn ein Bunker nicht mehr betrieben werden soll (z. B. weil das Dorf verloren ging), lösen Spieler ihre eigenen Bunker über den Button `Dissolve Bunker` auf.

![Dissolve Bunker Button](../assets/discordbot/bunker-information-system/18_dissolve_Bunker_button.png){ .screenshot }

Im Modal `Dissolve Bunker` gibst du die aufzulösenden Koordinaten ein — das Feld `Coordinate(s) / Text` akzeptiert auch mehrere Koordinaten oder Text mit eingebetteten Koordinaten. Nach einer Bestätigung entfernt der Bot die betroffenen Bunker aus der Datenbank und aktualisiert das Leader-View. TWU-Mods können fremde Bunker auf dem gleichen Weg auflösen.
...
Im zweiten Leader-View-Kanal `#⚫-leaderview-flexbunker` steht der Button `Flexbunker Control` zur Verfügung — er hilft, **versteckte Defensiv-Stärke** in eurem Stamm zu finden, also Dörfer, die laut Truppendaten genug Stärke haben, aber noch nicht als offizieller Bunker gemeldet sind.
...
Als Ergebnis bekommst du eine Tabelle aller Dörfer im Stamm, deren tatsächliche Dual-Stärke den Schwellwert erreicht — gruppiert nach Stamm und Spieler, absteigend nach Stärke sortiert. Genehmigte Bunker werden dabei ausgeblendet, sodass nur **zusätzliche** Flex-Kandidaten erscheinen.
...
- `Report-Link / Copy & Paste` — Eintrag über TW-Berichts-Link, `[report]`-Tag oder kopierten Berichtstext
- `Manual Entry` — manuelle Eingabe von Koordinate, Truppen und Info-Zeitpunkt
- `Admin: Remove Bunker` — Entfernen eines Feindbunker-Eintrags (nur TWU-Mod)
- `Export` — TSV-Export aller Feindbunker für Excel & Co.
...
Anschließend taucht der Eintrag in der Feindbunker-Übersicht im Kanal auf — gruppiert nach gegnerischem Stamm, dann nach Spieler, mit Koordinate, Truppenanzahl und Info-Zeitpunkt. Über `Admin: Remove Bunker` können TWU-Mods Einträge wieder löschen; `Export` liefert die komplette Liste als TSV.
