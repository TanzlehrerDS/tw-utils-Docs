...
![NeilB Tribe Info in the in-game Script Library](../assets/cleaner-fake-ut-tool/step1_07_script_library.png){ .screenshot }
...
overview (Ctrl+A, Ctrl+C). Alternatively the field also accepts the same data
as the file upload — header row included. That applies to both file formats,
including whatever "NeilsTribeInfo" puts on the clipboard via **"Copy CSV"** or
**"Copy JSON"**.
